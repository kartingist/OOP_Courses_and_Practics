...
ниже); метод to_bin, который возвращает двоичное представление числа в виде числа (может пригодиться функция bin) '''
...
class NewInt(int):
    ...
    def __init__(self, x):
        self.x = x
    def repeat(self, n=2):
        return int(str(self.x)*n)
    def to_bin(self):
        return int(bin(self.x)[2:])
